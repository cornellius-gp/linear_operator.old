...
class BlockLinearOperator(LinearOperator):
    ...
    Args:
        - :attr:`base_linear_operator` (LinearOperator or Tensor):
            Must be at least 3 dimenional.
        - :attr:`block_dim` (int):
            The dimension that specifies blocks.
    """

    def __init__(self, base_linear_operator, block_dim=-3):
        if base_linear_operator.dim() < 3:
            raise RuntimeError(
                "base_linear_operator must be a batch matrix (i.e. at least 3 dimensions - got "
                "{}".format(base_linear_operator.dim())
            )

        # Make sure block_dim is negative
        block_dim = block_dim if block_dim < 0 else (block_dim - base_linear_operator.dim())
        ...
        if block_dim != -3:
            positive_block_dim = base_linear_operator.dim() + block_dim
            base_linear_operator = base_linear_operator._permute_batch(
                *range(positive_block_dim),
                *range(positive_block_dim + 1, base_linear_operator.dim() - 2),
                positive_block_dim,
            )
    ...
    def _expand_batch(self, batch_shape):
        batch_shape = torch.Size((*batch_shape, self.base_linear_operator.size(-3)))
        res = self.__class__(self.base_linear_operator._expand_batch(batch_shape))
        return res

    def _getitem(self, row_index, col_index, *batch_indices):
        # First the easy case: just batch indexing
        if _is_noop_index(row_index) and _is_noop_index(col_index):
            return self.__class__(self.base_linear_operator._getitem(row_index, col_index, *batch_indices, _noop_index))
        ...
        new_base_linear_operator = self.base_linear_operator._getitem(row_index, col_index, *batch_indices)

        # Now construct a kernel with those indices
        return self.__class__(new_base_linear_operator, block_dim=-3)
    ...
    def _permute_batch(self, *dims):
        if torch.is_tensor(self.base_linear_operator):
            base_linear_operator = self.base_linear_operator.permute(*dims, -3, -2, -1)
        else:
            base_linear_operator = self.base_linear_operator._permute_batch(*dims, self.base_linear_operator.dim() - 3)
        res = self.__class__(base_linear_operator)
        return res

    def _unsqueeze_batch(self, dim):
        if torch.is_tensor(self.base_linear_operator):
            base_linear_operator = self.base_linear_operator.unsqueeze(dim)
        else:
            base_linear_operator = self.base_linear_operator._unsqueeze_batch(dim)
        res = self.__class__(base_linear_operator)
        return res
    ...
    def _mul_constant(self, other):
        ...
        return self.__class__(ConstantMulLinearOperator(self.base_linear_operator, other))

    def _transpose_nonbatch(self):
        return self.__class__(self.base_linear_operator._transpose_nonbatch())

    def zero_mean_mvn_samples(self, num_samples):
        res = self.base_linear_operator.zero_mean_mvn_samples(num_samples)
