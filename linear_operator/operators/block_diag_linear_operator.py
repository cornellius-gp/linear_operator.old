...
class BlockDiagLinearOperator(BlockLinearOperator):
    """
    Represents a linear operator that is the block diagonal of square matrices.
    ...
    @property
    def num_blocks(self):
        return self.base_linear_operator.size(-3)
    ...
    def _cholesky(self, upper=False):
        ...
        chol = self.__class__(self.base_linear_operator.cholesky(upper=upper))
    ...
    def _get_indices(self, row_index, col_index, *batch_indices):
        ...
        row_index_block = row_index // self.base_linear_operator.size(-2)
        col_index_block = col_index // self.base_linear_operator.size(-1)

        # Find the row/col index within each block
        row_index = row_index.fmod(self.base_linear_operator.size(-2))
        col_index = col_index.fmod(self.base_linear_operator.size(-1))

        # If the row/column blocks do not agree, then we have off diagonal elements
        # These elements should be zeroed out
        res = self.base_linear_operator._get_indices(row_index, col_index, *batch_indices, row_index_block)
    ...
    def _root_decomposition(self):
        return self.__class__(self.base_linear_operator._root_decomposition())

    def _root_inv_decomposition(self, initial_vectors=None):
        return self.__class__(self.base_linear_operator._root_inv_decomposition(initial_vectors))

    def _size(self):
        shape = list(self.base_linear_operator.shape)
        shape[-2] *= shape[-3]
        shape[-1] *= shape[-3]
        del shape[-3]
    ...
    def _solve(self, rhs, preconditioner, num_tridiag=0):
        if num_tridiag:
            ...
            rhs = self._add_batch_dim(rhs)
            res = self.base_linear_operator._solve(rhs, preconditioner, num_tridiag=None)
            res = self._remove_batch_dim(res)
            return res

    def diag(self):
        res = self.base_linear_operator.diag().contiguous()
        return res.view(*self.batch_shape, self.size(-1))

    def inv_quad_logdet(self, inv_quad_rhs=None, logdet=False, reduce_inv_quad=True):
        if inv_quad_rhs is not None:
            inv_quad_rhs = self._add_batch_dim(inv_quad_rhs)
        inv_quad_res, logdet_res = self.base_linear_operator.inv_quad_logdet(
            inv_quad_rhs, logdet, reduce_inv_quad=reduce_inv_quad
        )
        if inv_quad_res is not None and inv_quad_res.numel():
            if reduce_inv_quad:
                inv_quad_res = inv_quad_res.view(*self.base_linear_operator.batch_shape)
                inv_quad_res = inv_quad_res.sum(-1)
            else:
                inv_quad_res = inv_quad_res.view(*self.base_linear_operator.batch_shape, inv_quad_res.size(-1))
    ...
    def _svd(self) -> Tuple["LinearOperator", Tensor, "LinearOperator"]:
        U, S, V = self.base_linear_operator.svd()
    ...
    def _symeig(self, eigenvectors: bool = False) -> Tuple[Tensor, Optional[LinearOperator]]:
        evals, evecs = self.base_linear_operator.symeig(eigenvectors=eigenvectors)
        # Doesn't make much sense to sort here, o/w we lose the structure
