...
class BlockInterleavedLinearOperator(BlockLinearOperator):
    ...
    def _get_indices(self, row_index, col_index, *batch_indices):
        ...
        row_index_block = row_index.fmod(self.base_linear_operator.size(-3))
        col_index_block = col_index.fmod(self.base_linear_operator.size(-3))

        # Find the row/col index within each block
        row_index = row_index // self.base_linear_operator.size(-3)
        col_index = col_index // self.base_linear_operator.size(-3)
    ...
    def diag(self):
        block_diag = self.base_linear_operator.diag()
