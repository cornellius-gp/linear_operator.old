...
class SumBatchLinearOperator(BlockLinearOperator):
    """
    Represents a linear operator that is actually the sum of several linear operators blocks.
    ...
    Args:
        :attr:`base_linear_operator` (LinearOperator):
    ...
    def _add_batch_dim(self, other):
        ...
        shape.insert(-2, 1)
        expand_shape.insert(-2, self.base_linear_operator.size(-3))
        other = other.reshape(*shape).expand(*expand_shape)
        return other

    def _get_indices(self, row_index, col_index, *batch_indices):
        # Create an extra index for the summed dimension
        sum_index = torch.arange(0, self.base_linear_operator.size(-3), device=self.device)
        ...
        res = self.base_linear_operator._get_indices(row_index, col_index, *batch_indices, sum_index)
        return res.sum(-1)

    def _getitem(self, row_index, col_index, *batch_indices):
        res = self.base_linear_operator._getitem(row_index, col_index, *batch_indices, _noop_index)
